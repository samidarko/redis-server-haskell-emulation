...
-- group by from and to + idempotent
-- TODO test errors
main :: IO ()
main = hspec $ do
  describe "Redis.toRType" $ do
    ...
    describe "Errors" $ do
      ...
      it "toRType returns RError for Error" $ do
        toRType "-Error message\r\n" `shouldBe` RError "Error message"

      it "toRSString returns RError" $ do
        toRError "ERR unknown command 'foobar'" `shouldBe` RError "ERR unknown command 'foobar'"
    ...
    describe "Integers" $ do
      ...
      it "toRType returns RSString for a Simple String" $ do
        toRType ":5\r\nWorld" `shouldBe` RInt 5

      it "toRSString returns RSString" $ do
        toRInt "10\r\nWorld" `shouldBe` RInt 10
